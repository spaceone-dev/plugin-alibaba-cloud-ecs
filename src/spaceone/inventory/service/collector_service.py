...
FILTER_FORMAT = [
    {
        ...
        "change_rules": [
            {"resource_key": "data.compute.instance_id", "change_key": "instance_id"},
            {"resource_key": "data.compute.region", "change_key": "region_name"},
        ],
    },
    {
    ...
    {"key": "region_name", "name": "Region", "type": "list", "resource_type": "CUSTOM"},
]
...
class CollectorService(BaseService):
    def __init__(self, metadata):
        ...
        self.collector_manager: CollectorManager = self.locator.get_manager(
            "CollectorManager"
        )
    ...
    def list_resources(self, params):
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for mp_param in mp_params:
                future_executors.append(
                    executor.submit(self.collector_manager.list_resources, mp_param)
                )
            ...
            for future in concurrent.futures.as_completed(future_executors):
                try:
                    for result in future.result():
                        collected_region = (
                            self.collector_manager.get_region_from_result(result)
                        )
        ...
        print(
            f"############## TOTAL FINISHED {time.time() - start_time} Sec ##################"
        )
    ...
    def get_all_regions(self, secret_data, filter_region_name):
        ...
        return list(map(self._print_region_id, regions))
